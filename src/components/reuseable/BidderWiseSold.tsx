...
const addLogoToPage = (doc) => {
  // Define the image source (you should replace this with your logo image)
...
const BidderWiseSold = (sold,season,sale,company) => {
  const solddata =sold.filter((item)=>item.Status ===true)   
console.log(solddata);

  const tickets = solddata.sort((a, b) => a.expand.bidder_current.expand.reference.Company_name.localeCompare(b.expand.bidder_current.expand.reference.Company_name));

  console.log(tickets);
  const tableColumn = ["SL", "LOT", "INVOICE",  "FACTORY", "GRADE","BAG" ,"N.TOTAL", "OFFER","MAX","BIDDER", "REMARKS"];
  const tableRows = [];
  ...
  tickets.forEach((ticket, i) => {
    const ticketData = [
      ...
      ticket.Offer_price,
      ticket.price_max,
      ticket.expand.bidder_current.expand.reference.Company_name,
      ticket.Remarks,
    ];

    totalNetTotal += ticket.Grand_total;
    totalBagTotal += ticket.Package;
    tableRows.push(ticketData);
  });


  const totalRow = ["Total", "", "", "", "", totalBagTotal +' Bag',totalNetTotal.toFixed(2) +' kg', "", ""];
  ...
  doc.setProperties({
    title: "Bidder Wise Sold",
  });
  // doc.setFont('courier');
  doc.output("dataurlnewwindow");
};

export default BidderWiseSold;