...
const fetchCatalogData = async () => {
  ...
  const { data: catalogueData } = useSWR(
...
//   const [editMode, setEditMode] = useState(false);
  ...
  function truncateInvoiceNumber(invoiceNumber:any, maxLength:number) {
    ...
    //   setEditMode(!editMode)
    // }
    const delete_function = async(id)=>{
      try{
    ...
    return (
        ...
        <div className=" bg-white rounded-lg h-fit m-4 w-full col-span-2">
           
    <div className={`${edit_style}`}>
    {editMode&&<Edit  click={handle_edit_mode} data={selected} />}
   </div>
      ...
      {data1?.map((item, index) => {
        ...
        return(
           ...
           <td className="px-4 py-4 whitespace-nowrap flex  items-center justify-center text-sm font-medium">
            <a onClick={()=>handle_edit_mode(item)} className="text-black hover:text-black/50" href="#">
                <AiOutlineEdit />
            </a>
        </td>
        <td className="px-4 py-4 whitespace-nowrap flex items-center justify-center text-sm font-medium">
            <a onClick={()=>handle_delete(item.id)} className="text-red-500 hover:text-red-500" href="#"><RiDeleteBinLine /></a>
        </td>
           </td>
