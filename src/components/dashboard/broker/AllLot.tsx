...
useEffect(() => {
          ...
          // console.log(id);
            ...
            (async () => {
              try {
                ...
                await pb.collection("auction_info").update(`${id}`, data);
  ...
  const handle_un_sold = (id,status,pricemax) => {
   if(status){
    if (id) {
        const data = {
          price_max:pricemax,
    ...
    > */}
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
        <div className="flex items-center">
          <div onClick={()=>handle_sold(item.auc_id,item.status,item.price_max)} className=" cursor-pointer text-xl mr-4">
            <BsCheckSquareFill />
          </div>{" "}
          <div onClick={()=>handle_un_sold(item.auc_id,item.status,item.price_max)} className=" cursor-pointer text-2xl">
            <MdCancel />
          </div>
        </div>
      </td>
