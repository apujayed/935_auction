...
(async()=>{
  const min= minute+1;
  const page= Math.ceil(min/4);
  const response =await pb.collection('Eligibility').getFullList();
  const resultList8 = await pb.collection('catview').getList(1, 4,  {
  ...
  setCatalogueData(resultList8.items)
})()
},[])
function getValueBasedOnPattern(number) {
  const pattern = [0, 1, 2, 3];
  return pattern[number % 4];
}
useEffect(()=>{
  // const min2= minute+1;
  const index =  (getValueBasedOnPattern(minute));
  console.log(index);
  ...
  (async()=>{
    ...
    const page= Math.ceil(min/4);
    const response =await pb.collection('Eligibility').getFullList();
    const resultList8 = await pb.collection('catview').getList(page, 4,  {
        expand:'Factory,Warehouse,brokersID,bidder_current,bidder_current.reference',
        filter:`brokersID="${pb.authStore.model.id}" && Season = "${response[0].Season}" && Sale_number="${parseInt(response[0].Sale_Number)}" ` ,
        sort:'+created'       
    });
    setCatalogueData(resultList8.items[index])
    // console.log(resultList8.items[index]);
  ...
  const fetchCatalogData = async () => {
    const response =await pb.collection('Eligibility').getFullList();
    const resultList8 = await pb.collection('catview').getList(1, 10,  {
        expand:'Factory,Warehouse,brokersID,bidder_current,bidder_current.reference',
        filter:`brokersID="${pb.authStore.model.id}" && Season = "${response[0].Season}" && Sale_number="${parseInt(response[0].Sale_Number)}" ` ,
        sort:'+created'       
    });
    return resultList8.items;
  };
  ...
  return (
    ...
    <line x1="60.1" y1="60.1" x2="82.7" y2="82.7" stroke-linecap="round" stroke-linejoin="round" stroke-width="24">
      ...
      <Timer
              ...
              differ_time={differ_time}
            />
     <p>{catalogueData?.id}</p>
     <p>{catalogueData?.Lot_number}</p>
    </>
  );
}
